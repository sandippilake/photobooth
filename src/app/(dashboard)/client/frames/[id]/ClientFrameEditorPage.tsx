...
export default function ClientFrameEditorPage({ frameId, token, clientId }: { frameId: string; token: string; clientId: string }) {
  ...
  useEffect(() => {
    fetch(`${DIRECTUS_URL}/items/frames/${frameId}?fields=id,name,png_url,created_by,placeholder_schema,crop_rect`, {
      headers: { Authorization: `Bearer ${token}` }
    }).then(r => r.json()).then(data => {
      const f = data.data
      if (f.created_by !== clientId) { setError('You can only edit your own frames'); return }
      setFrame(f)
      setZones((f.placeholder_schema || []).map(normalizeZone))
      setCropRect(f.crop_rect || DEFAULT_CROP)
    })
  }, [frameId])
  ...
  if (error) return <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%' }}><p style={{ color: '#ef4444' }}>{error}</p></div>
  if (!frame) return <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%' }}><p style={{ color: '#6b7280' }}>Loading...</p></div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <div style={{ flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 24px', background: 'white', borderBottom: '1px solid #e5e7eb' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          <button onClick={() => router.push('/client/frames')}
